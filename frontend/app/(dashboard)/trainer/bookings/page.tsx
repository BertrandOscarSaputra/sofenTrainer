...
export default function TrainerBookingsPage() {
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      {success && <div className="p-4 rounded-xl bg-green-500/10 border border-green-500/20 text-green-400">{success}</div>}
        ...
        {confirmedSchedules.length === 0 ? (
          ...
          <div className="grid gap-4">
            {confirmedSchedules.map((booking) => (
              <Card key={booking.id} className="p-6 border border-white/5 bg-white/[0.02] flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 text-sm text-indigo-400 font-semibold mt-1">
                      <span className="flex items-center gap-1"><Clock size={14} /> {new Date(booking.scheduledAt).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</span>
                      <span>•</span>
                ...
                <div className="flex items-center gap-6">
                  ...
                  <Button variant="primary" size="sm" onClick={() => handleUpdateStatus(booking.id, 'DONE')}>
                    Selesai
                  </Button>
                </div>
              </Card>
        ...
        {pendingRequests.length === 0 ? (
          ...
          <div className="grid gap-4">
            {pendingRequests.map((booking) => (
              <Card key={booking.id} className="p-6 border border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4 flex-1">
                  <Avatar src={booking.userProfilePictureUrl} name={booking.userName || `User #${booking.userId}`} size="lg" />
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="text-lg font-bold text-white">{booking.userName || `User #${booking.userId}`}</h3>
                      <span className="px-2 py-0.5 rounded-md bg-yellow-500/20 text-yellow-500 text-[10px] font-black uppercase">PENDING</span>
                    </div>
                  <div className="flex flex-wrap gap-4 text-sm text-gray-400">
                    <span className="flex items-center gap-1"><Calendar size={14} /> {new Date(booking.scheduledAt).toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long' })}</span>
                    <span className="flex items-center gap-1"><Clock size={14} /> {new Date(booking.scheduledAt).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                  {booking.notes && <p className="mt-3 text-sm text-gray-400 italic bg-white/5 p-3 rounded-lg border border-white/5">"{booking.notes}"</p>}
                </div>
              </div>
                
              <div className="flex gap-2">
                  <button onClick={() => handleUpdateStatus(booking.id, 'CONFIRMED')} className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white text-sm font-bold rounded-xl transition-all">
                    Terima
                  </button>
                  <button onClick={() => handleUpdateStatus(booking.id, 'CANCELLED')} className="px-4 py-2 bg-white/5 hover:bg-white/10 text-gray-400 text-sm font-bold rounded-xl border border-white/10 transition-all">
                    Tolak
                  </button>
                </div>
              </Card>
              ...
              <Card key={booking.id} className={`p-5 border flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all ${
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase ${
                        booking.status === 'DONE' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'
                      }`}>
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                      <span className="flex items-center gap-1">
                        <Calendar size={12} /> 
                        {new Date(booking.scheduledAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
                      </span>
                      <span className="flex items-center gap-1">
                        <Clock size={12} />
                ...
                <div className="text-right flex flex-col items-end gap-1">
                   ...
                   <p className="text-xs text-gray-400">
                      {new Date(booking.bookedAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}
                   </p>
                </div>
              </Card>
            ))}
